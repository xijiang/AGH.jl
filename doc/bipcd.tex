...
\begin{tikzpicture}
  ...
  \node at (6, 1) {Very big $\mathbf{G}_{nn}$ on disk};
  \draw[red](0.01, 0.01) rectangle (3, 9);
  \node[red, rotate = 60] at (1.5, 4) {$\mathbf{Z}$, $\mathbf{ZZ}'\approx\mathbf{G}_{nn}$};
  \draw[cyan] (0, 9) rectangle (1, 0);
  \draw[cyan] (1, 8) rectangle (2, 0);
  \draw[cyan] (2, 7) rectangle (3, 0);
  \node[cyan, rotate = 90] at (.5, 3) {$\mathbf{z}$: One $\mathbf{Z}$ block of $n\times s$};
  \draw[blue, dashed] (1, 0) rectangle (9, 8);
  \node[blue, text width = 4cm] at (6, 3) {$\mathbf{g}$: Sub $\mathbf{G}$ to be updated and save to disk. Also blockwizely dealt.};
  \node at (5, 8.5) {\footnotesize Can be ignored};
  %\draw[dashed] (0, 6) -- (9, 6);
  %\draw[dashed, ] (3, 0) -- (3, 6);
